...
resource "aws_ecs_task_definition" "game_task" {
  family                   = var.game_task_famliy
  container_definitions    = <<DEFINITION
  [
    {
      "name": "${var.game_task_name}",
...
resource "aws_alb" "application_load_balancer" {
  name               = var.game_load_balancer_name
...
resource "aws_ecs_service" "game_service" {
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.target_group.arn
    container_name   = aws_ecs_task_definition.game_task.family
